...
# it seems that there is no need to use @everywhere when passing ENV VARS to workers
ENV["DEVITO_LANGUAGE"]="openmp"  # Enable OpenMP. 
ENV["OMP_NUM_THREADS"]="2"       # Number of OpenMP threads
# @everywhere ENV["DEVITO_LANGUAGE"]="openmp"  # Enable OpenMP. 
# @everywhere ENV["OMP_NUM_THREADS"]="2"       # Number of OpenMP threads

using Distributed, ClusterManagers

addprocs(SlurmManager(2), partition="debug", t="01:00:00")

#' # Import JUDI, Linear algebra utilities and Plotting
@everywhere using JUDI, PyPlot, LinearAlgebra
